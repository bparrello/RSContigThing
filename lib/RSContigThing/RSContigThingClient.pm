package RSContigThing::RSContigThingClient;
...
RSContigThing::RSContigThingClient
...
sub new
{
    ...
    my $self = {
	client => RSContigThing::RSContigThingClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a RSContigThing.FilterContigsParams
$return is a RSContigThing.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a RSContigThing.workspace_name
	contigset_id has a value which is a RSContigThing.contigset_id
	min_length has a value which is an int
workspace_name is a string
contigset_id is a string
FilterContigsResults is a reference to a hash where the following keys are defined:
	new_contigset_ref has a value which is a RSContigThing.ws_contigset_id
...
# Authentication: required
	...
	method => "RSContigThing.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for RSContigThing::RSContigThingClient\n";
    }
    if ($sMajor == 0) {
        warn "RSContigThing::RSContigThingClient version is $svr_version. API subject to change.\n";
    }
}
...
package RSContigThing::RSContigThingClient::RpcClient;
