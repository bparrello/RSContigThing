package RSContigThing::RSContigThingServer;
...
our %async_run_methods = (
        'filter_contigs_async' => 'RSContigThing.filter_contigs',
);

our %async_check_methods = (
        'filter_contigs_check' => 'RSContigThing.filter_contigs',
);
...
sub call_method {
    ...
    my $ctx = RSContigThing::RSContigThingServerContext->new($self->{loggers}->{userlog},
...
{
    # Service RSContigThing requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for RSContigThing but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = RSContigThing::RSContigThingServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service RSContigThing.");
    }
...
package RSContigThing::RSContigThingServerContext;

use strict;

=head1 NAME

RSContigThing::RSContigThingServerContext
...
package RSContigThing::RSContigThingServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = RSContigThing::RSContigThingServer->new(
